...
import { Settings as SettingsIcon, User, Palette, Sun, Moon, AlertTriangle, Trash2 } from 'lucide-react'
...
const Settings = ({ user, setUser }) => {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      showMessage('success', 'تم حفظ الملف الشخصي بنجاح! ✔️')
    } catch (error) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
      ...
      <div className="card !p-4 sm:!p-6 md:!p-8">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          <button
            ...
            className={`p-4 sm:p-6 rounded-lg border-2 transition-all flex flex-col items-center gap-2 ${
      ...
      <div className="card !p-4 sm:!p-6 md:!p-8 border-2 border-danger">
        <h2 className="text-base sm:text-lg md:text-xl font-bold mb-3 sm:mb-4 text-danger flex items-center gap-2">
        ...
        <div className="space-y-3 sm:space-y-4">
          <button 
            ...
            className="w-full bg-red-50 text-danger px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-red-100 font-semibold text-sm sm:text-base flex items-center justify-center gap-2"
          >
            <Trash2 size={20} />
            حذف جميع المهام
          </button>
          <button 
            ...
            className="w-full bg-danger text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-red-600 font-semibold text-sm sm:text-base flex items-center justify-center gap-2"
          >
            <Trash2 size={20} />
            حذف الحساب نهائياً
          </button>
